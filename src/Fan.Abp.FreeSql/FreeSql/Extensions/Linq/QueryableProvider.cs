...
namespace FreeSql.Extensions.Linq
{
    ...
    public class QueryableProvider<TCurrent, TSource> : FreeSqlQueryProvider, IOrderedQueryable<TCurrent>
    {
        ...
        public ISelect<TSource> AsSelect()
        {
            return _select;
        }
    ...
    public class QueryProvider<TCurrent, TSource> : FreeSqlQueryProvider, IQueryProvider
    {
        ...
        public IQueryable<TElement> CreateQuery<TElement>(Expression expression)
        {
            ...
            if (typeof(TElement) != typeof(TCurrent))
                return new QueryableProvider<TElement, TSource>(expression,
                    new QueryProvider<TElement, TSource>(_select, expression), _select);
        ...
        public object ExecuteExp(Expression expression, Type tresult, bool isProcessed)
        {
            ...
            {
                object throwCallExp(string message) => throw new Exception(
                    $"解析失败 {callExp.Method.Name} {message}，提示：可以使用扩展方法 IQueryable.RestoreToSelect() 还原为 ISelect 再查询");
                ...
                object tplMaxMinAvgSum(string method)
                {
                    if (callExp.Arguments.Count == 2)
                    {
                        ...
                        return Utils.GetDataReaderValue(tresult,
                            _select.GetType().GetMethod(method).MakeGenericMethod(avgParam.ReturnType)
                                .Invoke(_select, new object[] { avgParam }));
                    }
